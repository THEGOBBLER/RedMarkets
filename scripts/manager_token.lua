...
function onDrop(tokenCT, draginfo)
	...
	if nodeCT and draginfo.getType() == "sRoll" then
		return RMDamage.onDrop("ct", ActorManager.getActorFromCT(nodeCT), draginfo);
	elseif nodeCT then
...
function updateHealthBarScale(tokenCT, nodeCT)
	...
	if widgetHealthBar then
		local nPercentWounded,_,_ = fGetHealthInfo(nodeCT);
